    ...
    initial begin
        ...
        begin
            fork
                ...
                begin
                    // Receive sequentially to avoid race on M_RVALID
                    mst_recv_r(cap_sid1, cap_rdata[0], cap_resp1, cap_rlast);
                    $display("[%0t] MST recv R1: sid=0x%03x data=0x%08h",
                             $time, cap_sid1, cap_rdata[0]);
                    mst_recv_r(cap_sid2, cap_rdata[1], cap_resp2, cap_rlast);
                    $display("[%0t] MST recv R2: sid=0x%03x data=0x%08h",
                             $time, cap_sid2, cap_rdata[1]);
                end
            join
        end
        ...
        //=================================================================
        // TEST 7: Back-to-back R + B interleaved
        //=================================================================
        $display("\n--- TEST 7: B+R interleaved ---");
        begin
            fork
                ...
                begin
                    mst_recv_b(cap_sid2, cap_resp2);
                    $display("[%0t] MST recv B: sid=0x%03x resp=%0d", $time, cap_sid2, cap_resp2);
                end
            join
        end
